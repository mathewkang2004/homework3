...
q4_table <- ols_results %>%
  ...
  mutate(`Star Rating Category` = case_when(
    `Star Rating Category` == "(Intercept)" ~ "2.5 Stars or Below (Baseline)",
...
knitr::kable(q4_table, digits = 4, 
             caption = "OLS Regression of Market Share on Star Ratings") %>%
    kable_styling(latex_options = c("striped", "hold_position"))
```
...
The estimates appear to be relatively stable as the bandwidth increases for both star rating comparisons. However, the ATE appears to decrease sharply when the bandwidth increases from 0.14 to 0.15 for the 3.0-star versus 3.5-star comparison.
...
The topmost McCrary Density Plot shows the distribution of the running variable around the 2.75-star threshold, while the bottom-most plot shows the distribution of the running variable around the 3.25-star threshold, with the x-axis representing the centered raw score and y-axis representing the density.

In the top-most plot, there is a massive spike in observations immediately to the left of the 2.75-star threshold, followed by a larger drop in density estimates to the right. The t-statistic is also about -74 with a p-value of 0, which strongly rejects the null hypothesis of continuity. This suggests overwhelming evidence of manipulation and that plans around the threshold are unlikely to be sorted randomly.

Likewise, the bottom-most plot fails the McCrary Density Test. While there is a noticeably smaller spike in observations to the left of the threshold, there is still an sharp drop immediately before the threshold. The t-statistic is also about 5.7 with a p-value of 0, which, again, strongly rejects the null hypothesis of continuity. Therefore, there is significant evidence of manipulation for both thresholds.
