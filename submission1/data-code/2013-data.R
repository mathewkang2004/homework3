...
landscape.2013 <- mapd.clean.merge(ma.data=ma.data, mapd.data=mapd.data, y)
...
ma.2013 <- plan.data.2013 %>%
  inner_join(service.data.2013 %>% select(contractid, fips),
             by = c("contractid","fips")) %>%
  filter(!state %in% c("VI","PR","MP","GU","AS",""),
         snp == "No",
         (planid < 800 | planid >= 900),
         !is.na(planid), !is.na(fips)) %>%
  left_join(pen.2013 %>% ungroup() %>% rename(state_long = state, county_long = county) %>%
...
ma.2013 <- ma.2013 %>%
  mutate(year = coalesce(as.numeric(year.x), as.numeric(year.y), 2013)) %>%
  select(-starts_with("year."))

full.2013 <- ma.2013 %>%
  left_join(state.2013, by = "state") %>%
  mutate(ssa = as.character(ssa)) %>%
  left_join(landscape.2013 %>% mutate(state=str_to_lower(state)), by = c("contractid","planid","state_name" = "state","county")) %>%
  left_join(star.ratings.2013 %>% select(-any_of(c("contract_name", "org_type", "org_marketing"))),
            by = c("contractid")) %>%
  mutate(
  ...
  left_join(benchmark.2013 %>% filter(!is.na(ssa)), by = c("ssa")) %>%
  select(-starts_with("year.")) %>%
  mutate(year = y) %>%
  mutate(
    ma_rate = case_when(
...
# Save data ---------------------------------------------------------------
write_csv(full.2013,"data/output/data-2013.csv")
