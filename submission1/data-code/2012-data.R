...
landscape.2012 <- mapd.clean.merge(ma.data=ma.data, mapd.data=mapd.data, y)
...
ma.2012 <- ma.2012 %>%
  mutate(year = coalesce(as.numeric(year.x), as.numeric(year.y), 2012)) %>%
  select(-starts_with("year."))

full.2012 <- ma.2012 %>%
  left_join(state.2012, by = "state") %>%
  mutate(ssa = as.character(ssa)) %>%
  left_join(landscape.2012 %>% mutate(state=str_to_lower(state)), by = c("contractid","planid","state_name" = "state","county")) %>%
  left_join(star.ratings.2012 %>% select(-any_of(c("contract_name", "org_type", "org_marketing"))),
            by = c("contractid")) %>%
  mutate(
  ...
  left_join(benchmark.2012 %>% filter(!is.na(ssa)), by = c("ssa")) %>%
  select(-starts_with("year.")) %>%
  mutate(year = y) %>%
  mutate(
    ma_rate = case_when(
...
# Save data ---------------------------------------------------------------
write_csv(full.2012,"data/output/data-2012.csv")
