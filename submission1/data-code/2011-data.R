...
landscape.2011 <- mapd.clean.merge(ma.data=ma.data, mapd.data=mapd.data, y)
...
full.2011 <- ma.2011 %>%
  ...
  left_join(landscape.2011 %>% mutate(state=str_to_lower(state)), by = c("contractid","planid","state_name" = "state","county")) %>%
